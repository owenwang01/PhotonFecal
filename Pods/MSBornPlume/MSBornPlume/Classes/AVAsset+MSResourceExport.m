...
- (void)ms_loadDurationOfAsset:(void(^)(CMTime duration))completionHandler {
...
- (void)generatePreviewImagesWithMaxItemSize:(CGSize)itemSize count:(NSUInteger)count completionHandler:(void (^)(AVAsset * _Nonnull, NSArray<UIImage *> * _Nullable, NSError * _Nullable))block {
    ...
    __weak typeof(self) _self = self;
    [self ms_loadDurationOfAsset:^(CMTime duration) {
        __strong typeof(_self) self = _self;
        if ( !self ) return;
    ...
    [self.exportSession exportAsynchronouslyWithCompletionHandler:^{
        ...
        switch ( self.exportSession.status ) {
            ...
            case AVAssetExportSessionStatusCompleted: {
                UIImage *image = [compositionM ms_screenshotWithTime:kCMTimeZero];
                dispatch_async(dispatch_get_main_queue(), ^{
                    __strong typeof(_self) self = _self;
                    if ( !self ) return;
...
- (void)ms_screenshotWithTime:(NSTimeInterval)t
                         size:(CGSize)size
...
@implementation AVAsset (MSResourceExport)
#pragma mark -

- (_SJAVAssetPreviewImagesGenerator *)ms_imagesGenerator {
...
- (void)ms_generatePreviewImagesWithMaxItemSize:(CGSize)itemSize count:(NSUInteger)count completionHandler:(void (^)(AVAsset * _Nonnull, NSArray<UIImage *> * _Nullable, NSError * _Nullable))block {
    [self.ms_imagesGenerator generatePreviewImagesWithMaxItemSize:itemSize count:count completionHandler:block];
}
...
- (_SJAVAssetExporter *)ms_assetExporter {
...
- (void)ms_exportWithStartTime:(NSTimeInterval)startTime
                      duration:(NSTimeInterval)secs1
                        toFile:(NSURL *)fileURL
                    presetName:(nullable NSString *)presetName
                      progress:(nullable void(^)(AVAsset *a, float progress))progress
                       success:(nullable void(^)(AVAsset *a, AVAsset * __nullable sandboxAsset, NSURL * __nullable fileURL, UIImage * __nullable thumbImage))success
                       failure:(nullable void(^)(AVAsset *a, NSError * __nullable error))failure {
    [self.ms_assetExporter exportWithStartTime:startTime duration:secs1 toFile:fileURL presetName:presetName progress:progress success:success failure:failure];
}
...
- (_SJAVAssetScreenshotGenerator *)ms_screenshotGenerator {
...
- (UIImage * __nullable)ms_screenshotWithTime:(CMTime)time {
    return [self.ms_screenshotGenerator ms_screenshotWithTime:time];
}
...
- (void)ms_screenshotWithTime:(NSTimeInterval)time
    ...
    [self ms_screenshotWithTime:time size:CGSizeZero completionHandler:block];
}

- (void)ms_screenshotWithTime:(NSTimeInterval)time
                         size:(CGSize)size
            completionHandler:(void(^)(AVAsset *a, UIImage * __nullable image, NSError *__nullable error))block {
    [self.ms_screenshotGenerator ms_screenshotWithTime:time size:size completionHandler:block];
}

- (void)ms_cancelScreenshotOperation {
    _SJAVAssetScreenshotGenerator *_Nullable generator = objc_getAssociatedObject(self, _cmd);
    [generator ms_cancelScreenshotOperation];
}

#pragma mark -
- (_SJAVAssetGIFGenerator *)ms_GIFGenerator {
...
- (void)ms_generateGIFWithBeginTime:(NSTimeInterval)beginTime
                       ...
                       imageMaxSize:(CGSize)size
                           ...
                           progress:(void(^)(AVAsset *a, float progress))progressBlock
                            success:(void(^)(AVAsset *a, UIImage *GIFImage, UIImage *thumbnailImage))successBlock
                            failure:(void(^)(AVAsset *a, NSError *error))failureBlock {
    [self.ms_GIFGenerator ms_generateGIFWithBeginTime:beginTime duration:duration imageMaxSize:size interval:interval toFile:fileURL progress:progressBlock success:successBlock failure:failureBlock];
}
