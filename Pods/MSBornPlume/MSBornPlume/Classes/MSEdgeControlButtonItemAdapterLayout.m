//
//  MSEdgeControlButtonItemAdapterLayout.m
//  Pods
//
...
@interface MSEdgeControlButtonItemAdapterLayout () {
    NSMutableArray<MSEdgeControlButtonItemLayoutAttributes *> *_layoutAttributes;
}
@end

@implementation MSEdgeControlButtonItemAdapterLayout
...
- (void)prepareLayout {
    ...
    switch ( _layoutType ) {
        case MSAdapterLayoutTypeVerticalLayout:
            [self _prepareLayout_Vertical];
            break;
        case MSAdapterLayoutTypeHorizontalLayout:
            [self _prepareLayout_Horizontal];
            break;
        case MSAdapterLayoutTypeFrameLayout:
...
- (void)_prepareLayout_Horizontal {
    ...
    for ( NSInteger i = 0 ; i < _items.count ; ++ i ) {
        CGFloat width = 0;
        MSEdgeControlButtonItem *item = _items[i];
        if ( item.fill )
            [fillIndexes addObject:@(i)];
        else if ( item.isHidden ) { }
        else if ( 0 != item.size )
            width = item.size;
        else if ( item.placeholderType == MSButtonItemPlaceholderType_49x49 )
            width = height;
        else if ( item.customView ) {
            if ( item.placeholderType == MSButtonItemPlaceholderType_49xAutoresizing ) {
    ...
    for ( NSInteger index = 0 ; index < _items.count ; ++ index ) {
        MSEdgeControlButtonItem *item = _items[index];
        current_x += item.insets.front;
        MSEdgeControlButtonItemLayoutAttributes *attrs = [MSEdgeControlButtonItemLayoutAttributes layoutAttributesForItemWithIndex:index];
...
- (void)_prepareLayout_Frame {
    ...
    for ( NSInteger i = 0 ; i < _items.count ; ++ i ) {
        ...
        else if ( item.placeholderType == MSButtonItemPlaceholderType_49x49 || item.image != nil ) {
...
@implementation MSEdgeControlButtonItemLayoutAttributes
+ (instancetype)layoutAttributesForItemWithIndex:(NSInteger)index {
    MSEdgeControlButtonItemLayoutAttributes *attrs = MSEdgeControlButtonItemLayoutAttributes.alloc.init;
    attrs.index = index;
    return attrs;
}
