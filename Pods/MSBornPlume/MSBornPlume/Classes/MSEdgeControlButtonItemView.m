...
@interface MSEdgeControlButtonItemView ()
...
@implementation MSEdgeControlButtonItemView
...
- (void)reloadItemIfNeeded {
    ...
    // 2.
        ...
        // set image
        if ([_item.image isKindOfClass:[UIImage class]]) {
            if ( _item.image != _itemImageView.image ) {
                _itemImageView.image = _item.image;
            }
        } else if ([_item.image isKindOfClass:[NSURL class]]) {
            [_itemImageView sd_setImageWithURL:_item.image];
        }
    }
    // 3.
