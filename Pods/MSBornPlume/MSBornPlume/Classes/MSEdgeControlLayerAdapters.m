...
@interface MSEdgeControlLayerAdapters ()
@property (nonatomic, readonly) BOOL isFitOnScreen;
@end

@implementation MSEdgeControlLayerAdapters {
    id _notifyToken;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _screen = (struct MS_Screen){max, min, _isIPhoneXSeries()};
...
- (MSPlumeControlMaskView *)topContainerView {
    if ( _topContainerView ) return _topContainerView;
    _topContainerView = [[MSPlumeControlMaskView alloc] initWithStyle:MSMaskStyle_top];
...
- (MSPlumeControlMaskView *)bottomContainerView {
    if ( _bottomContainerView ) return _bottomContainerView;
    _bottomContainerView = [[MSPlumeControlMaskView alloc] initWithStyle:MSMaskStyle_bottom];
...
- (MSEdgeControlButtonItemAdapter *)topAdapter {
    if ( _topAdapter ) return _topAdapter;
    _topAdapter = [MSEdgeControlButtonItemAdapter.alloc initWithFrame:CGRectZero layoutType:MSAdapterLayoutTypeHorizontalLayout];
    [self.topContainerView addSubview:_topAdapter.view];
    [self _updateTopLayout:nil];
    return _topAdapter;
}

- (MSEdgeControlButtonItemAdapter *)leftAdapter {
    if ( _leftAdapter ) return _leftAdapter;
    _leftAdapter = [[MSEdgeControlButtonItemAdapter alloc] initWithFrame:CGRectZero layoutType:MSAdapterLayoutTypeVerticalLayout];
...
- (MSEdgeControlButtonItemAdapter *)bottomAdapter {
    if ( _bottomAdapter ) return _bottomAdapter;
    _bottomAdapter = [[MSEdgeControlButtonItemAdapter alloc] initWithFrame:CGRectZero layoutType:MSAdapterLayoutTypeHorizontalLayout];
...
- (MSEdgeControlButtonItemAdapter *)rightAdapter {
    if ( _rightAdapter ) return _rightAdapter;
    _rightAdapter = [[MSEdgeControlButtonItemAdapter alloc] initWithFrame:CGRectZero layoutType:MSAdapterLayoutTypeVerticalLayout];
...
- (MSEdgeControlButtonItemAdapter *)centerAdapter {
    if ( _centerAdapter ) return _centerAdapter;
    _centerAdapter = [[MSEdgeControlButtonItemAdapter alloc] initWithFrame:CGRectZero layoutType:MSAdapterLayoutTypeFrameLayout];
    _centerAdapter.itemFillSizeForFrameLayout = self.bounds.size;
