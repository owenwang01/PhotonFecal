...
@implementation NSObject (MSKVOHelper)
- (void)ms_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath {
    [self ms_addObserver:observer forKeyPath:keyPath context:NULL];
}

- (void)ms_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath context:(nullable void *)context {
    [self ms_addObserver:observer forKeyPath:keyPath options:NSKeyValueObservingOptionOld | NSKeyValueObservingOptionNew context:context];
}
...
- (void)ms_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(nullable void *)context {
    ...
    NSMutableSet *set = [self ms_observerhashSet];
    ...
    [observer ms_addDeallocCallbackTask:^(id  _Nonnull obj) {
        if ( _set == nil ) return;
        dispatch_semaphore_wait(lock, DISPATCH_TIME_FOREVER);
...
- (NSMutableSet<NSString *> *)ms_observerhashSet {
...
@implementation NSObject (MSNotificationHelper)
- (void)ms_observeWithNotification:(NSNotificationName)notification target:(id _Nullable)target usingBlock:(void(^)(id self, NSNotification *note))block {
    [self ms_observeWithNotification:notification target:target queue:NSOperationQueue.mainQueue usingBlock:block];
}
- (void)ms_observeWithNotification:(NSNotificationName)notification target:(id _Nullable)target queue:(NSOperationQueue *_Nullable)queue usingBlock:(void(^)(id self, NSNotification *note))block {
    ...
    [self ms_addDeallocCallbackTask:^(id  _Nonnull obj) {
...
#pragma mark - MSDeallocCallback
@interface __SJDeallockCallback : NSObject {
@public
    char _key;
}
@property (nonatomic, unsafe_unretained, nullable) id target;
@property (nonatomic, copy, nullable) MSDeallockCallbackTask task;
@end
...
- (void)ms_addDeallocCallbackTask:(MSDeallockCallbackTask)block {
...
@property (nonatomic, copy, readonly) MSKVOObservedChangeHandler hanlder;
@end
@implementation __SJKVOObserver
- (instancetype)initWithTarget:(__unsafe_unretained id)target
                    forKeyPath:(NSString *)keyPath
                       options:(NSKeyValueObservingOptions)options
                        change:(MSKVOObservedChangeHandler)hanlder {
    self = [super init];
    if ( !self ) return nil;
    _target = target;
...
MSKVOObserverToken __attribute__((overloadable))
sjkvo_observe(id target, NSString *keyPath, NSKeyValueObservingOptions options, MSKVOObservedChangeHandler handler) {
    ...
    return (MSKVOObserverToken)&observer->_key;
}
